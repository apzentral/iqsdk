...
, methods: {
  ...
  , select: function (table, fields, where, opts) {
      ...
      try {
        ...
        this.debug(query);
        return new iQ.data.SqlRecordSet(this.db.execute(query), fields);
      } catch (ex) {
  ...
  , processUpdates: function (updates) {
      ...
      try {
        ...
        var installedUpdates = iQ.data.PropertiesStore.getArray(this.my.PROPERTYSTORE_UPDATES.format(this.name));
  ...
  , mergeUpdate: function (updateInfo, update) {
      ...
      try {
        ...
        iQ.data.PropertiesStore.setArray(this.my.PROPERTYSTORE_UPDATES.format(this.name), installedUpdates);
